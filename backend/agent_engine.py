"""
SubSync Agent Engine
Uses Hermes AIAgent loop with custom tools and skills
"""
import os
import sys
import json
...
"""
    skill_discovery = """
## Skill Library (Lazy Discovery)
You have access to a learned skill library for this agent. Skill descriptions
are NOT pre-loaded into this prompt — you must discover them explicitly to
save tokens.

**Before starting any multi-step task, call `skills_list()` first** to see if
a relevant playbook already exists. If the description matches the user's
request, load the full body with `skill_view(name="<skill-name>")` and follow
it. Skills encode the canonical tool sequence and output format for recurring
SubSync workflows.

Skip `skills_list()` only for:
- Trivial single-tool lookups (e.g., "get customer 5's email")
- Pure conversational turns with no tool use

For anything that sounds like a workflow ("audit...", "review...", "check...",
"investigate...", "report...", "escalate..."), always check skills first.
"""
    return skill + "\n\n" + tools_desc + "\n" + skill_discovery
...
class SubSyncAgent:
    ...
    def run(self, user_message: str, conversation_history: list = None) -> dict:
        ...
        try:
            ...
            # ── Disable Level 1 skill injection (system-prompt bloat).
            #    Hermes' stock behavior injects every skill's name + description
            #    into the system prompt at session start, costing ~80 tokens per
            #    skill × every turn. We replace that with a lazy Level 2/3
            #    discovery model: the agent calls skills_list() when it thinks
            #    a skill may be relevant. The skill_discovery section of our
            #    system prompt (see build_system_prompt) nudges it to do so.
            #
            #    To revert to Level 1 behavior: `cp agent_engine_level1.py
            #    agent_engine.py` and restart. Keep agent_engine_level1.py as
            #    the reference snapshot of the pre-change implementation.
            try:
                from agent import prompt_builder as _pb
                if not getattr(_pb, "_subsync_level1_disabled", False):
                    _pb._orig_build_skills_system_prompt = _pb.build_skills_system_prompt
                    _pb.build_skills_system_prompt = lambda *a, **kw: ""
                    _pb._subsync_level1_disabled = True
                    print("  🧹 [SubSync] Level 1 skill injection disabled (lazy discovery mode)")
            except Exception as _e:
                print(f"  ⚠️  [SubSync] Could not disable Level 1 injection: {_e}")
